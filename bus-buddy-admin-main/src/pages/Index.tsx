...
import { Bus, FileText, Ticket, ChevronRight, Route, Building2, BarChart3 } from 'lucide-react';
import { format } from 'date-fns';

const Index = () => {
  const { vehicles, vouchers, routes, terminal, getVehicleById } = useBookingStore();
    ...
    <div className="min-h-screen bg-background">
      ...
      <header className="bg-gradient-to-br from-primary/10 via-background to-background border-b">
        <div className="container max-w-4xl mx-auto px-4 py-8">
          <div className="flex items-center justify-between">
            ...
            {/* Terminal Info */}
            {terminal && (
              <div className="hidden sm:flex items-center gap-3 px-4 py-3 rounded-xl bg-card border">
                <div className="w-10 h-10 rounded-lg bg-primary/10 flex items-center justify-center">
                  <Building2 className="w-5 h-5 text-primary" />
                </div>
                <div className="text-right">
                  <p className="font-semibold leading-tight">{terminal.name}</p>
                  <p className="text-sm text-muted-foreground">{terminal.city}, {terminal.area}</p>
                </div>
              </div>
            )}
          </div>
      ...
      <main className="container max-w-4xl mx-auto px-4 py-8">
        ...
        <div className="grid gap-4 md:grid-cols-2 mb-8">
          <Link to="/vehicles" className="nav-button">
          ...
          <Link to="/vouchers" className="nav-button">
            <div className="w-16 h-16 rounded-2xl bg-primary/10 flex items-center justify-center">
